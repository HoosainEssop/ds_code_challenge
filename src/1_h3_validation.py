...
def validation_run():
    ...
    if os.path.exists(f"validation_files/{VALIDATION_FILE_C1.replace('geojson', 'csv')}"):
        validation_df = pd.read_csv(f"validation_files/{VALIDATION_FILE_C1.replace('geojson', 'csv')}", index_col=False)
        ...
        os.makedirs("validation_files")
        validation_df.to_csv(f"validation_files/{VALIDATION_FILE_C1.replace('geojson', 'csv')}", index=False)
